...
export default observer(function CircleForm ({ className }) {
  ...
  const [isOpen, toggleOpen] = useStoredToggle('di-circle')

  return <Form store={circleForm} className={cn('circle-form', className)}>
    <div className="flex items-center justify-between cursor-pointer" onClick={toggleOpen}>
      <h2 className="font-medium text-alt flex items-center">
        <PlusIcon className="size-5 mr-1" /> CIRCLE
      </h2>
    ...
    <Collapsible isOpen={isOpen} openClassName="overflow-visible">
      ...
      >
        <PlusIcon className="size-5 mr-1" /> ADD CIRCLE
      </Btn>
    </Collapsible>
  </Form>
})
